...
app.get('/api/server/ping', (req, res) => {
  const host = req.query.host || bot.currentStatus.server;
  const port = parseInt(req.query.port || bot.currentStatus.port || 25565);
  const timer = setTimeout(() => res.json({ state: 'timeout', host, port }), 8000);
  try {
    mc.ping({ host, port, closeTimeout: 6000 }, (err, result) => {
      clearTimeout(timer);
      if (err) {
        const offline = err.code === 'ECONNREFUSED' || err.code === 'ENOTFOUND';
        return res.json({ state: offline ? 'offline' : 'timeout', host, port, error: err.message });
      }
      let desc = '';
      try {
        if (typeof result.description === 'string') desc = result.description;
        else if (result.description?.text) desc = result.description.text;
        else if (result.description?.extra) desc = result.description.extra.map(e => e.text||'').join('');
        else desc = JSON.stringify(result.description||'');
      } catch(_) {}
      const isStarting = /starting|preparing|loading|waking|hibernat|please wait/i.test(desc);
      res.json({
        state: isStarting ? 'starting' : 'online',
        host, port,
        motd: desc.replace(/§./g,'').trim(),
        players: { online: result.players?.online??0, max: result.players?.max??0 },
        version: result.version?.name ?? '',
        latency: result.latency ?? null,
      });
    });
  } catch(e) { clearTimeout(timer); res.json({ state: 'timeout', host, port, error: e.message }); }
});
...
function App() {
  ...
  const pingServer=async()=>{
    ...
    try{
      const r=await fetch('/api/server/ping?host='+encodeURIComponent(status.server)+'&port='+status.port);
      const d=await r.json();
      setPingResult(d);
    }catch(e){setPingResult({state:"timeout",error:e.message});}
  ...
  const RightPanel=()=><div className="term-scroll" style={{background:"var(--term-surface)",overflowY:"auto",height:"100%",display:"flex",flexDirection:"column"}}>
    ...
    <section style={{padding:12,borderBottom:"1px solid var(--term-border)"}}>
      ...
      <button onClick={pingServer} disabled={pinging} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"5px 0",borderRadius:4,fontSize:11,fontFamily:"monospace",fontWeight:"bold",background:"#1f3a5f",color:"#79c0ff",border:"1px solid #1f6feb",cursor:"pointer"}}>
        <McItemImage name="compass" size={14}/> {pinging?"ĐANG PING...":"PING SERVER"}
      </button>
      {pingResult&&<div style={{marginTop:6,padding:"6px 8px",borderRadius:4,fontSize:10,fontFamily:"monospace",border:"1px solid",
        borderColor:pingResult.state==="online"?"#238636":pingResult.state==="starting"?"#9e6a03":"#58181c",
        background:pingResult.state==="online"?"#0d2a16":pingResult.state==="starting"?"#2a1f03":"#2a0d0d",
        color:pingResult.state==="online"?"#3fb950":pingResult.state==="starting"?"#e3a84b":"#f85149"}}>
        {pingResult.state==="online"&&<>
          <div>✅ ONLINE — {pingResult.players?.online}/{pingResult.players?.max} người</div>
          {pingResult.motd&&<div style={{color:"#c9d1d9",marginTop:2}}>{pingResult.motd}</div>}
          {pingResult.version&&<div style={{color:"#586069"}}>{"v"+pingResult.version+(pingResult.latency!=null?" · "+pingResult.latency+"ms":"")}</div>}
        </>}
        {pingResult.state==="starting"&&<>
          <div>⏳ ĐANG KHỞI ĐỘNG</div>
          {pingResult.motd&&<div style={{color:"#c9d1d9",marginTop:2}}>{pingResult.motd}</div>}
        </>}
        {pingResult.state==="offline"&&<div>🔴 OFFLINE — server đã tắt</div>}
        {pingResult.state==="timeout"&&<div>⚠️ TIMEOUT — không phản hồi</div>}
      </div>}
    </section>
    {/* Quick Tasks */}
    <section style={{padding:12,borderBottom:"1px solid var(--term-border)"}}>
