...
function QBtn({ label, cmd, item, disabled, onCmd }) {
  ...
  return <button onClick={()=>onCmd(cmd)} disabled={disabled}
    ...
    <McItemImage name={item} size={18}/><span>{label}</span>
  </button>;
}
...
function DropPanel({ onCmd, disabled }) {
  ...
  return <section style={{padding:12,borderBottom:"1px solid var(--term-border)"}}>
    ...
    <div style={{display:"flex",flexWrap:"wrap",gap:4,marginTop:6}}>
      {[{q:"all",i:"chest"},{q:"gỗ",i:"oak_log"},{q:"đá",i:"cobblestone"},{q:"cát",i:"sand"},{q:"đất",i:"dirt"}].map(({q,i})=>(
...
function App() {
  ...
  const connectWs=useCallback(()=>{
    ...
    ws.onclose=()=>{setWsState("closed");reTimer.current=setTimeout(connectWs,3000);};
    ws.onerror=()=>ws.close();
  },[]);
  ...
  useEffect(()=>{
    if(autoScroll) logsEnd.current?.scrollIntoView({behavior:"smooth"});
  },[logs,autoScroll]);
  ...
  const quickCmd=msg=>api("/bot/command",{msg,type:"command"});
  const toggle=f=>api("/bot/toggle",{feature:f});
  const sendInput=()=>{ const msg=input.trim(); if(!msg)return; api("/bot/command",{msg,type:inputMode}); setInput(""); };
    ...
    <section style={{padding:12,borderBottom:"1px solid var(--term-border)"}}>
      ...
      <div style={{display:"flex",gap:6}}>
        <button onClick={startBot} disabled={status.running} style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"6px 0",borderRadius:4,fontSize:11,fontFamily:"monospace",fontWeight:"bold",background:"#238636",color:"#fff",border:"none",cursor:"pointer"}}>
          <McItemImage name="lime_wool" size={14}/>START
        </button>
        <button onClick={stopBot} disabled={!status.running} style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"6px 0",borderRadius:4,fontSize:11,fontFamily:"monospace",fontWeight:"bold",background:"#da3633",color:"#fff",border:"none",cursor:"pointer"}}>
          <McItemImage name="red_wool" size={14}/>STOP
        </button>
      </div>
    </section>
    {/* Quick Tasks */}
    ...
    <section style={{padding:12,borderBottom:"1px solid var(--term-border)"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:4}}>
        ...
        <button onClick={()=>quickCmd("dừng")} disabled={!status.online} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,padding:"6px 0",borderRadius:4,fontSize:11,fontFamily:"monospace",fontWeight:"bold",background:"#b91c1c",color:"#fff",border:"none",cursor:"pointer",width:"100%"}}>
          <McItemImage name="redstone_block" size={14}/>DỪNG TẤT CẢ
        </button>
      </div>
    </section>
  ...
  const ConsolePanel=()=><div style={{display:"flex",flexDirection:"column",flex:1,minWidth:0,minHeight:0}}>
    ...
    {/* Input bar */}
    <div style={{flexShrink:0,display:"flex",alignItems:"center",gap:8,padding:"8px 12px",borderTop:"1px solid var(--term-border)",background:"var(--term-surface)"}}>
      ...
      <span style={{fontSize:14,fontFamily:"monospace",color:inputMode==="command"?"var(--term-cyan)":"var(--term-purple)"}}>{inputMode==="command"?"►":"#"}</span>
      <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendInput()}
        placeholder={inputMode==="command"?"chặt gỗ / đào đá / theo [name] / dừng …":"Gửi chat đến Minecraft…"}
        autoFocus
        style={{flex:1,background:"transparent",border:"none",fontSize:13,fontFamily:"monospace",color:"var(--term-white)"}}/>
      <button onClick={sendInput} disabled={!input.trim()} style={{padding:"4px 12px",borderRadius:4,fontSize:11,fontFamily:"monospace",fontWeight:"bold",background:inputMode==="command"?"#1f6feb":"#8957e5",color:"#fff",border:"none",cursor:"pointer"}}>SEND</button>
    </div>
  </div>;
