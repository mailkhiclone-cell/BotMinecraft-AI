...
// ── CẤU HÌNH ──────────────────────────────────────────────────────
const CONFIG = {
  host:       process.env.BOT_HOST      || 'abcdxyz.aternos.me',
  port:       parseInt(process.env.BOT_PORT  || 'here'),
  username:   process.env.BOT_USERNAME  || 'here',
  version:    process.env.BOT_VERSION   || '1.21.11',
  groqApiKey: process.env.GROQ_API_KEY  || 'here',
};
// ──────────────────────────────────────────────────────────────────
...
function createBot() {
  try {
    bot = mineflayer.createBot({ host:CONFIG.host, port:CONFIG.port, username:CONFIG.username, version:CONFIG.version, auth:'offline' });
    ...
    bot.on('forcedMove', () => {
      if (!botOnline) { botOnline = true; logJ('Bot đã vào thế giới (forcedMove)!'); }
    ...
    bot.once('health', () => {
      if (!botOnline) { botOnline = true; logJ('Bot đã vào thế giới (health)!'); }
    });
    ...
    bot.on('kicked', (reason) => {
      ...
      lastError = `Bị kick: ${r.slice(0,120)}`;
    ...
    bot.on('error', (err) => { lastError = err.code==='ECONNRESET'||err.code==='ECONNREFUSED'?`Lỗi mạng: ${err.code} — server có thể offline`:err.message; logE(lastError); handleRejoin(); });
    bot.on('end', () => { botOnline = false; logW('Mất kết nối (End). Rejoin...'); handleRejoin(); });
  } catch(err) {
