  ...
  while (!stopTask) {
    ...
    if (!block) {
      ...
      if (notFoundStreak >= 4) {
        ...
        const targetY = pos.y > 20 ? Math.max(pos.y - 30, -50) : pos.y;
...
function handleRejoin() {
  ...
  if (rejoinAttempts < MAX_REJOIN) {
    rejoinAttempts++;
    // Progressive backoff: 2s → 4s → 8s → 16s → 32s → 45s (cap)
    // Giúp Aternos có đủ thời gian wake up trước khi thử lại
    const delay = Math.min(REJOIN_DELAY * Math.pow(2, rejoinAttempts - 1), 45000);
    logW(`Mất kết nối. Thử lại [${rejoinAttempts}/${MAX_REJOIN}] sau ${(delay/1000).toFixed(0)}s...`);
...
function createBot() {
  try {
    ...
    // Đếm giây chờ để người dùng biết bot không bị đơ
    let spawnWaitSecs = 0;
    ...
    spawnWaitInterval = setInterval(() => {
      ...
      spawnWaitSecs += 30;
      logW(`Đang chờ server tải... (${spawnWaitSecs}s) — Aternos có thể mất 3-5 phút`);
    }, 30000);

    // Timeout 5 phút — Aternos thường mất 3-5 phút để wake up hoàn toàn
    spawnTimeout = setTimeout(() => {
      if (!botOnline) {
        if (spawnWaitInterval) { clearInterval(spawnWaitInterval); spawnWaitInterval = null; }
        logW('Chờ spawn quá 5 phút — thử rejoin...');
        handleRejoin();
      }
    }, 300000);
    ...
    bot.on('kicked', (reason) => {
      ...
      if (isAnotherLocation) {
        ...
        // Dùng handleRejoin() để dọn interval + progressive backoff, nhưng đặt delay tối thiểu 10s
        rejoinAttempts = Math.max(rejoinAttempts, 3); // ép delay >= 8s để tránh spam
