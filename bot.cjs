...
async function equipToolForBlock(block) {
  ...
  for (const m of TOOL_PRI) { const tool = invItems.find(i => i.name===`${m}_${tt}`); if (tool) { try { await bot.equip(tool,'hand'); return; } catch(e){} } }
}
...
// Đặt nước phía sau bot (giữa bot và địch) để làm chậm người đuổi
async function tryPlaceWaterBucket(enemyPos) {
  if (!bot || !mcData) return false;
  const bucketInfo = mcData.itemsByName['water_bucket'];
  if (!bucketInfo) return false;
  const bucket = bot.inventory.findInventoryItem(bucketInfo.id, null, false);
  ...
  try {
    ...
    bot.activateItem();
    logS('[PvP] 🪣 Đặt xô nước để làm chậm kẻ thù!');
...
// Tìm water source block gần nhất và múc vào bucket (sau khi flee xong)
async function tryPickUpWater() {
  if (!bot || !mcData) return false;
  const emptyBucketInfo = mcData.itemsByName['bucket'];
  if (!emptyBucketInfo) return false;
  const emptyBucket = bot.inventory.findInventoryItem(emptyBucketInfo.id, null, false);
  if (!emptyBucket) return false; // không có bucket rỗng

  // Tìm water source block (metadata 0 = source) gần bot trong 10m
  const waterBlock = bot.findBlock({
    matching: b => b && b.name === 'water' && b.metadata === 0,
    maxDistance: 10,
  });
...
async function throwEnderPearl(targetPos) {
  ...
  try {
    ...
    await bot.lookAt(targetPos.offset(0, 1, 0), true);
    bot.activateItem();
...
async function shootBowWithLead(targetEntity, rangedItem) {
  ...
  try {
    ...
    const leadPos = tPos.offset(vel.x * flightTicks, vel.y * flightTicks * 0.4, vel.z * flightTicks);
...
async function mineBlockType(blockName, who) {
  ...
  const isOreMining = blockIds.some(id => {
    const b = Object.values(mcData.blocksByName).find(bl => bl.id === id);
...
const FARM_FRUITS = ['melon','pumpkin','sugar_cane'];
...
async function depositToChest(who) {
  ...
  for (let r = 8; r <= 64; r += 8) {
    const found = bot.findBlock({ matching: chestIds, maxDistance: r });
    if (found && !allChests.some(c => c.position.equals(found.position))) allChests.push(found);
  }
...
async function startDuel(playerName) {
  ...
  const trackInterval = setInterval(async () => {
    ...
    // ══════════════════════════════════════════════════════════════
    // 1. YẾU MÁU (HP < 6 = 3 tim): CHẠY TRỐN + ĐẶT NƯỚC + ĂN
    // ══════════════════════════════════════════════════════════════
    if (myHp < 6) {
      if (!isCombatFleeing) {
      ...
      if (now - lastWaterPlace > 3000) {
        ...
        else lastWaterPlace = now; // reset dù không có nước, tránh spam
      }
    ...
    // Đã hồi phục (HP >= 10): múc lại nước, trang bị, tiếp tục chiến
    if (isCombatFleeing && myHp >= 10) {
    ...
    // ══════════════════════════════════════════════════════════════
    // 4. ENDER PEARL: GAP CLOSE khi địch xa (> 12m, không chạy)
    // ══════════════════════════════════════════════════════════════
    ...
    // ══════════════════════════════════════════════════════════════
    if (hDist > 6 && now - lastBowShot > 1800) {
      const ranged = bot.inventory.items().find(i => i.name==='crossbow' || i.name==='bow');
      if (ranged) {
        await shootBowWithLead(e, ranged);
        lastBowShot = now;
      ...
      if (isPvpChallenge) {
        ...
        if (fullyReady) {
          ...
          const denyMsg = await getAI(
            ...
            `Bot Minecraft từ chối vì máu chưa đầy, hứa đấu khi máu full, 1 câu ngắn tiếng Việt cá tính.`
          );
...
async function goSleep(who) {
  ...
  const beds = [];
  for (let r = 8; r <= 64; r += 8) {
    const found = bot.findBlock({ matching: BED_BLOCKS, maxDistance: r });
    if (found && !beds.some(b => b.position.equals(found.position))) beds.push(found);
  }
  if (!beds.length) {
