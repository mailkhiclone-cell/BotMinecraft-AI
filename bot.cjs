...
function createBot() {
  try {
    ...
    // Timeout cảnh báo nếu spawn không về sau 120s (Aternos có thể mất 1-2 phút để load)
    spawnTimeout = setTimeout(() => {
      if (!botOnline) {
        logW('Chờ spawn quá 120s — server có thể đang tải hoặc cần whitelist. Thử rejoin...');
        handleRejoin();
      }
    }, 120000);
